...
export function getSinglePost(slug) {
    ...
    const [htmlContent] = Transformer.getHtmlFromMarkdown(fileContent)
    // console.log("==================================")
    //console.log("hrmlcontents and backlinks")
    return {
        id: slug,
        name: currentFilePath.split("/").pop().replace('.md', ''),
...
export function getSingleCanvas(slug) {
    // List of filenames that will provide existing links to wikilink
    let currentFilePath = toFilePath(slug)
    //console.log("currentFilePath: ", currentFilePath)
    var fileContent = Node.readFileSync(currentFilePath)
    ...
    // console.log("===============\n\nFile is scanning: ", slug)
    const graph = Transformer.getGraphFromCanvas(fileContent)
    // console.log("==================================")
    //console.log("hrmlcontents and backlinks")
    return {
        id: slug,
...
export const allFiles = getAllMarkdownFiles();
...
function getSlugHashMap() {
    ...
    slugMap["/"] = Node.getMarkdownFolder() + "/🌳 Древо.canvas";
...
export function getAllSlugs() {
    ...
    allSlugs = filePaths.map(f => toSlug(f))
    return allSlugs;
}

export function getDirectoryData() {
    console.log('get dir data')
    return dirTree(Node.getMarkdownFolder(), {extensions: /\.(md|canvas)/});
}
...
export function convertObject(thisObject) {
    ...
    const routerPrefix = routerPath && routerPath.endsWith(".canvas") ? "/canvas/" : "/note/";
    routerPath = routerPath ? routerPrefix + routerPath : null;
    const newObject = {
    ...
    if (thisObject.children != null && thisObject.children.length > 0) {
        ...
        newObject.children.sort((a, b) =>
          (a.routePath || '') > (b.routePath || '') ? 1 : (b.routePath || '') > (a.routePath || '') ? -1 : 0
        );
        return newObject;
    } else {
        return newObject
