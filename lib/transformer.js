    ...
    getGraphFromCanvas: function (content) {
        ...
        if (data) {
            const nodes = data.nodes
                .filter((node) => { return node.type == "file" })
                .map((node) => {
                const nodeType = node.type == "file" && node.file.endsWith(".md") ? "link" : node.type;
                const text = nodeType == "link" ? node.file.split("/").pop().replace(".md", "") : node.text;
                const value = nodeType == "link" ? "/note/" + toSlug([Node.getMarkdownFolder(), node.file].join("/")) : "";
